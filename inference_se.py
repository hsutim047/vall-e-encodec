...
def nar_decode(model, tokenizer, inputs, batch_code, layer=0):
    base_input = inputs
    batch_code = np.array(batch_code).transpose((1, 0, 2))
    pad_widths = [(0, 0), (0, 7 - batch_code.shape[1]), (0, 0)]
    batch_code = np.pad(batch_code, pad_widths, mode='constant', constant_values=tokenizer.pad_token_id)
    base_input["decoder_input_ids"] = batch_code.tolist()
...
def cascade_ar_nar(ar_model, nar_model, ar_tokenizer, nar_tokenizer, dataset, device):
    ...
    for layer in range(1, 8):
        ...
        inputs = inputs.to(device)
        layer_list.append(nar_decode(nar_model, nar_tokenizer, inputs, layer_list, layer))
...
def nar_model_only(model, tokenizer, dataset, device):
    ...
    tgt_encodec_input = tokenizer(
        "".join([f"v_tok_{u}" for u in dataset['tgt_encodec_0'][0]]),
        return_tensors='pt', add_special_tokens=False)
    ...
    for layer in range(1, 8):
        ...
        inputs = inputs.to(device)
        layer_list.append(nar_decode(model, tokenizer, inputs, layer_list, layer))
...
def main(args):
    ...
    dataset = dataset.filter(lambda x : len(x['src_encodec_0']) <= 700)
    ...
    if args.cascade_ar_nar:
        ...
        nar_model = BartEncodecForConditionalGeneration.from_pretrained(args.nar_checkpoint)
        nar_model.to(device)
